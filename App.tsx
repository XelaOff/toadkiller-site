...
import { getRouteHref, getRoutePath } from './utils/routes';
...
const App: React.FC = () => {
  ...
  const isMissions = currentPath === '/missions';
  const isAdmin = currentPath === '/admin/missions';
  ...
  return (
    <div className="min-h-screen relative flex flex-col overflow-x-hidden text-[#fefce8]">
      ...
      {/* Top Nav */}
      <div className="fixed top-10 left-4 right-4 z-50 flex justify-between items-center gap-3">
        {/* Logo - Left */}
        <a href={getRouteHref('/')} className="flex items-center">
        ...
        <div className="flex flex-wrap justify-end gap-2 md:gap-3">
          <a
            href={getRouteHref('/missions')}
            className={`group ${isMissions ? 'bg-yellow-400 text-[#166534]' : 'bg-green-500 hover:bg-green-400 text-green-900'} font-cartoon px-4 py-3 rounded-2xl flex items-center gap-2 transition-all hover:scale-105 shadow-[0_6px_0_#166534] active:translate-y-1 active:shadow-none`}
          >
            <span className="text-xl leading-none">M</span>
            <span className="translate-y-0.5 hidden sm:inline">Missions</span>
          </a>
          <a
            href={getRouteHref('/admin/missions')}
            className={`group ${isAdmin ? 'bg-yellow-400 text-[#166534]' : 'bg-green-500 hover:bg-green-400 text-green-900'} font-cartoon px-4 py-3 rounded-2xl hidden md:flex items-center gap-2 transition-all hover:scale-105 shadow-[0_6px_0_#166534] active:translate-y-1 active:shadow-none`}
          >
            <span className="text-xl leading-none">A</span>
            <span className="translate-y-0.5">Admin</span>
          </a>
          <a
